...
4. Environment Variables:
   ```
   DATABASE_URL=<PostgreSQL Internal URL with +asyncpg driver>
   ...
   ALLOWED_ORIGINS=*
   ```
   **Important**: Change `postgresql://` to `postgresql+asyncpg://` in DATABASE_URL
...
3. Configuration:
   - **Name**: `fullfil-worker`
   - **Environment**: `Docker`
   - **Docker Command**: `celery -A app.celery_app worker --loglevel=info -Q default`
...
- Visit your web service URL (e.g., `https://fullfil-web.onrender.com`)
- Check `/docs` for API documentation
- Test CSV upload with a small file first
...
PROD-001,Widget A,High-quality widget,29.99
PROD-002,Widget B,Premium widget,49.99
PROD-003,Gadget X,Innovative gadget,99.99
```
...
- [ ] Check products appear in Products tab
